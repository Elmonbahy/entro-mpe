...
use App\Exports\Gudang\MutationExport;
use App\Exports\Gudang\KartuStockExport;
...
class MutationController extends Controller
{
  public function kartuStock(Request $request)
  {
    ...
    return view('pages.mutation.gudang.kartu-stock', [
  ...
  public function exportExcelKartuStock(Request $request)
  {
    $request->validate([
      'tgl_awal' => 'nullable|date',
      'tgl_akhir' => 'nullable|date|after_or_equal:tgl_awal',
    ...
    $filename = 'kartu-stock.xlsx';

    if ($barang_id) {
      $barang = Barang::find($barang_id);
      if ($barang) {
        $safeNamaBarang = preg_replace('/[^a-zA-Z0-9_\-]/', '_', $barang->nama);
        $filename = 'Kartu-stock' . '_' . $safeNamaBarang . '.xlsx';
      }
    }

    return Excel::download(
      new KartuStockExport($tgl_awal, $tgl_akhir, $barang_id),
      $filename
    );
  }
  ...
  public function index(Request $request)
  {
    ...
    return view('pages.mutation.gudang.index', [
      'mutations' => $mutations,
      'tgl_awal' => $request->tgl_awal,
      'tgl_akhir' => $request->tgl_akhir,
    ]);
