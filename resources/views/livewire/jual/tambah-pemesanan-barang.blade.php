...
@script
  <script>
    let brandSelectEl = new TomSelect('#brand_id', {
      onChange(value) {
        Livewire.dispatch('Jual.TambahPemesananBarang:onBrandChange', {
          id: value
        });
      }
    });
    ...
    let barangSelectEl = new TomSelect('#barang_id', {
      onChange(value) {
        Livewire.dispatch('Jual.TambahPemesananBarang:onBarangChange', {
          id: value
        });
      }
    });

    Livewire.on('Jual.TambahPemesananBarang:created', () => {
      brandSelectEl.clear()
      barangSelectEl.clear()
      barangSelectEl.clearOptions()
    })

    Livewire.on('Jual.TambahPemesananBarang:brandChanged', ([res]) => {
      barangSelectEl.clear();
      barangSelectEl.clearOptions();
