...
@section('content')
  @foreach ($jual_details as $jual_detail)
    <div class="p-4">
      <!-- Header -->
      <div
      ...
      <div class="text-center pb-1 mt-2">
        ...
        <p class="text-center">No: {{ $jual->nomor_faktur }}</p>
      </div>

      <!-- Jenis QC -->
      <table style="width: 100%; font-family: sans-serif; font-size: 12px; text-align: center; margin: 0 auto;">
        <tr>
          <td><span class="checkbox-box"></span> QC Barang Masuk</td>
          <td><span class="checkbox-box"></span> QC Barang Keluar</td>
          <td><span class="checkbox-box"></span> QC Barang Retur</td>
        </tr>
      </table>
      ...
      <table style="font-family: sans-serif; font-size: 13px; width: 100%;" class="mt-1">
        <tr>
          ...
          <td style="border-bottom: 1px dotted #000;"><strong>{{ $jual_detail->barang->nama ?? '' }}</strong></td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td>:</td>
          <td style="border-bottom: 1px dotted #000;"><strong>{{ $jual_detail->batch ?? '' }}</strong></td>
        </tr>
        <tr>
          <td><strong>Expired Date </strong><sup>*)</sup></td>
          <td>:</td>
          <td style="border-bottom: 1px dotted #000;">
            <strong>{{ $jual_detail->tgl_expired ? \Carbon\Carbon::parse($jual_detail->tgl_expired)->format('d/m/Y') : '' }}</strong>
          </td>
        </tr>
      </table>
