...
@section('content')
  <div class="container-fluid px-4">
    ...
    <x-page-header title="Laporan Barang Masuk" class="mb-3">
    </x-page-header>

    <div class="card mb-3">
      <div class="p-3 card-header d-flex align-items-center">
        <p class="mb-0 fw-semibold me-2">Form Laporan Barang Masuk</p>
        <x-info-tooltip
...
• Status faktur opsional: Proses gudang / Selesai." />
      </div>

      <div class="p-3">
        <form method="GET" action="{{ route('gudang.laporan-beli.index') }}" autocomplete="off">
          <div class="row">
    ...
    @if ($data->isEmpty())
      <div class="card">
        <div class="p-3 card-header">
          <p class="mb-0 fw-semibold">Tabel Laporan Barang Masuk</p>
        </div>
        <div class="card-body">
    ...
    @if ($data->isNotEmpty())
      <div class="card">
        <div class="p-3 card-header d-flex justify-content-between align-items-center">
          <p class="mb-0 fw-semibold">Tabel Laporan Barang Masuk</p>
          <a href="{{ route('gudang.laporan-beli.excel') }}?tgl_awal={{ $tgl_awal }}&tgl_akhir={{ $tgl_akhir }}&supplier_id={{ $supplier_id }}&status_faktur={{ request('status_faktur') }}"
            ...
            <table class="table table-bordered small text-center">
              ...
              <tbody>
                @foreach ($data as $faktur)
                  @php
                    $rowspan = $faktur['beli_details_count'] + 1;
                  @endphp
