...
• Sales dan Status bayar opsional." />
      ...
      <div class="p-3">
        <form method="GET" action="{{ route('keuangan.laporan-list-faktur-jual.index') }}" autocomplete="off">
          <div class="row">
    ...
    @if ($data->isNotEmpty())
      <div class="card">
        <div class="p-3 card-header d-flex justify-content-between align-items-center">
          ...
          <a href="{{ route('keuangan.laporan-list-faktur-jual.excel') }}?tgl_awal={{ request('tgl_awal') }}&tgl_akhir={{ request('tgl_akhir') }}&pelanggan_id={{ request('pelanggan_id') }}&sales_id={{ $sales_id }}&status_bayar={{ request('status_bayar') }}"
