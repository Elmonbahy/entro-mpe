    ...
    @if ($mutations->isNotEmpty())
      <div class="card">
        <div class="p-3 card-header d-flex justify-content-between align-items-center">
          <p class="mb-0 fw-semibold">Tabel mutasi</p>
          <a href="{{ route('gudang.mutation.excel-mutation') }}?tgl_awal={{ $tgl_awal }}&tgl_akhir={{ $tgl_akhir }}"
              ...
              <tbody>
                @foreach ($mutations as $item)
                  <tr>
                    ...
                    <td>
                      {{ number_format($item['total_stock_awal'], 0, ',', '.') }}
                    </td>
                    <td>
                    ...
                    <td>
                      {{ number_format($item['total_stock_keluar'], 0, ',', '.') }}
                    </td>
                    <td>
                      {{ \Number::currency($item['total_harga_jual'], 'IDR', 'id_ID') }}
                    </td>
                    <td>
                      {{ number_format($item['total_stock_retur_beli'], 0, ',', '.') }}
                    </td>
                    <td>
                      {{ number_format($item['total_stock_retur_jual'], 0, ',', '.') }}
                    </td>
                    <td>
                      {{ number_format($item['total_stock_rusak'], 0, ',', '.') }}
                    </td>
                    <td>
                      {{ number_format($item['sisa_stock'], 0, ',', '.') }}
                    </td>
                    <td>
                      {{ $item['barang_satuan'] }}
                    </td>
