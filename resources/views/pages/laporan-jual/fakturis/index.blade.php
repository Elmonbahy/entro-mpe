...
@section('content')
  <div class="container-fluid px-4">
    ...
    <x-page-header title="Laporan Profit Jual" class="mb-3">
    </x-page-header>

    <div class="card mb-3">
      <div class="p-3 card-header d-flex align-items-center">
        <p class="mb-0 fw-semibold me-2">Form Laporan Profit Jual</p>
        <x-info-tooltip
...
• Sales dan Status bayar opsional." />
      ...
      <div class="p-3">
        <form method="GET" action="{{ route('fakturis.laporan-jual.index') }}" autocomplete="off">
          <div class="row">
    ...
    @if ($data->isEmpty())
      <div class="card">
        <div class="p-3 card-header">
          <p class="mb-0 fw-semibold">Tabel Laporan Profit Jual</p>
        </div>
        <div class="card-body">
    ...
    @if ($data->isNotEmpty())
      <div class="card">
        <div class="p-3 card-header d-flex justify-content-between align-items-center">
          <p class="mb-0 fw-semibold">Tabel Laporan Profit Jual</p>
          <a href="{{ route('fakturis.laporan-jual.excel') }}?tgl_awal={{ $tgl_awal }}&tgl_akhir={{ $tgl_akhir }}&pelanggan_id={{ $pelanggan_id }}&sales_id={{ $sales_id }}"
              ...
              <tbody>
                @foreach ($data as $faktur)
                  ...
                  <tr>
                    ...
                    <td rowspan="{{ $faktur['jual_details_count'] + 1 }}">
                      {{ number_format($faktur['persen'], 0, ',', '.') }}%</td>
                    <td rowspan="{{ $rowspan }}">{{ number_format($faktur['ongkir'], 0, ',', '.') }}</td>
                    <td class="fw-bold" rowspan="{{ $rowspan }}">
                      {{ number_format($faktur['net_profit'], 0, ',', '.') }}</td>
                  </tr>
                  {{-- end jual --}}
                  ...
                  @foreach ($faktur['jual_details'] as $item)
                    <tr>
                      ...
                      {{-- tagihan --}}
                      <td class="text-end">{{ number_format($item['hna_jual'], 0, ',', '.') }}</td>
                      {{-- HNA jual --}}
                      <td class="text-end">{{ number_format($item['ppn_jual'], 0, ',', '.') }}</td>
                      {{-- PPN jual --}}
                      <td class="text-end">{{ number_format($item['hna_beli'], 0, ',', '.') }}</td>
                      {{-- HNA  beli --}}
                      <td class="text-end">{{ number_format($item['ppn_beli'], 0, ',', '.') }}</td>
                      {{-- PPN beli --}}
                      <td class="text-end">{{ number_format($item['profit'], 0, ',', '.') }}</td> {{-- profit --}}
                    </tr>
                  @endforeach
                  {{-- end jual detail --}}
                  ...
                  <tr class="fw-bold text-end">
                    ...
                    <td>{{ number_format($faktur['total_hna_jual'], 0, ',', '.') }}</td>
                    <td>{{ number_format($faktur['total_ppn_jual'], 0, ',', '.') }}</td>
                    <td>{{ number_format($faktur['total_hna_beli'], 0, ',', '.') }}</td>
                    <td>{{ number_format($faktur['total_ppn_beli'], 0, ',', '.') }}</td>
                    <td>{{ number_format($faktur['total_profit'], 0, ',', '.') }}</td>
                    {{-- % --}}
                  </tr>
