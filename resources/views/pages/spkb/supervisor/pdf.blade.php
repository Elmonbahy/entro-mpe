...
@section('content')
  <div class="p-4">
    ...
    <div class="text-center pb-1 mt-2">
      <p class="fw-bold mb-0" style="font-size: 15px;">SURAT PERINTAH KELUAR BARANG</p>
      <p>Nomor Faktur : <strong>{{ $jual->nomor_faktur }}</strong></p>
    ...
    <table style="width: 100%; margin-bottom: 15px;">
      <tr>
        ...
        <td style="vertical-align: top; width: 60%;">
          <p>Pelanggan.</p>
          <p class="fw-bold">{{ $jual->pelanggan->nama }}</p>
          <p>{{ $jual->pelanggan->kota }}</p>
          <p>{{ $jual->pelanggan->alamat }}</p>
          <p>{{ $jual->pelanggan->contact_person }}</p>
          <p>{{ $jual->pelanggan->contact_phone }}</p>
        </td>
    ...
    <p class="mb-2">
      Mohon segera dapat disiapkan barang pesanan sebagai berikut: <strong>{{ $jual->nomor_pemesanan }}</strong>
    </p>
      ...
      <tbody>
        @foreach ($jual_detail as $item)
          <tr>
            ...
            <td width="40">{{ $item->barang->brand->nama }}</td>
            <td width="150">{{ $item->barang->nama }}</td>
            <td width="30" class="text-center">{{ $item->barang->kode ?? '-' }}</td>
            ...
            <td width="30">{{ $item->barang->nie }}</td>
          </tr>
        @endforeach
      </tbody>
    </table>
