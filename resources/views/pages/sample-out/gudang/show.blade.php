...
@section('content')
  <div class="container-fluid px-4">
    <x-page-header title="Sampel Keluar" class="mb-3">
      <a href="{{ route('gudang.sample-out.pdf', ['id' => $sample_out->id]) }}" class="btn btn-primary" target="_blank">
    ...
    <div class="card mt-3">
      <div class="card-header p-3 d-flex justify-content-between align-items-center">
        ...
        @if ($sample_out->status_sample == \App\Enums\StatusSample::PROCESS_GUDANG)
          <form method="POST" action="{{ route('gudang.sample-out.done', ['id' => $sample_out->id]) }}"
            onsubmit="return confirm('Apakah sampel barang keluar sudah sesuai?')">
            @csrf
            @method('PATCH')
                  ...
                  <tr>
                    ...
                    <td>
                      <div class="d-flex gap-2">
                        ...
                        @can('retur', $item)
                          <a href="{{ route('gudang.sample-out.retur-item', ['sample_out_detail_id' => $item->id, 'id' => $sample_out->id]) }}"
                            class="btn btn-warning" title="retur barang sampel keluar">
                            <i class="bi bi-arrow-counterclockwise"></i>
                          </a>
                        @endcan

                        @can('stock', $item)
                          <a href="{{ route('gudang.sample-out.stock-item', ['sample_out_detail_id' => $item->id, 'id' => $sample_out->id]) }}"
                            class="btn btn-primary" title="stock sampel barang keluar">
                ...
                @foreach ($returs as $item)
                  <tr>
                    ...
                    <td>
                      ...
                      <form action="{{ route('gudang.sample-out.updateKeterangan', $item->id) }}" method="POST"
                    ...
                    <td>
                      @if ($item->is_diganti && !$item->diganti_at)
                        <a href="{{ route('gudang.sample-out.retur-done', ['id' => $item->id]) }}"
